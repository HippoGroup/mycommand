...
public class MapperScannerConfigurer
     ...
     *
     * @param commandSessionBeanName Bean name of the {@code SqlSessionTemplate}
     * @since 1.1.0
     */
     ...
     * it is too early to build mybatis object instances.
     *
     * @param commandSessionFactoryBeanName Bean name of the {@code SqlSessionFactory}
     * @since 1.1.0
     */
...
//    scanner.setSqlSessionTemplate(this.sqlSessionTemplate);
        scanner.setCommandSessionFactoryBeanName(this.commandSessionFactoryBeanName);
        scanner.setCommandSessionBeanName(this.commandSessionBeanName);
    ...
    private void processPropertyPlaceHolders() {
        ...
        if (!prcs.isEmpty() && applicationContext instanceof ConfigurableApplicationContext) {
            ...
            this.commandSessionFactoryBeanName = getPropertyValue("commandSessionFactoryBeanName", values);
            this.commandSessionBeanName = getPropertyValue("commandSessionBeanName", values);
            this.lazyInitialization = getPropertyValue("lazyInitialization", values);
            this.defaultScope = getPropertyValue("defaultScope", values);
        }
        this.basePackage = Optional.ofNullable(this.basePackage).map(getEnvironment()::resolvePlaceholders).orElse(null);
        this.commandSessionFactoryBeanName = Optional.ofNullable(this.commandSessionFactoryBeanName)
                .map(getEnvironment()::resolvePlaceholders).orElse(null);
        this.commandSessionBeanName = Optional.ofNullable(this.commandSessionBeanName)
                .map(getEnvironment()::resolvePlaceholders).orElse(null);
